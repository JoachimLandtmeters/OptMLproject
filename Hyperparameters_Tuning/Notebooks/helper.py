...
def hyperparameters_tuning_LBFGS_minibatch(trainset, valset, batchsize_grid, history_size_grid, epochs, model_NN,max_iter_= 10,fullbatch=False,line_search=True):
    ...
    for bs in batchsize_grid:
        ...
        for hs in history_size_grid:
            ...
            if model_NN=="FCNN":
                ...
                optimizer=optim.LBFGS(model.parameters(),max_iter=max_iter_,history_size=hs, line_search_fn='strong_wolfe')
            if model_NN=="FCNN2":
                #here max_iter = history size
                sizes = [input_size,128,64,output_size]
                model = fully_connected_NN(sizes)
                criterion = nn.NLLLoss()
                ...
                if line_search==False:
                    linesearch=None
                optimizer=optim.LBFGS(model.parameters(),max_iter=max_iter_,history_size=hs,line_search_fn=linesearch)
            elif model_NN=="CNN2":
                model=ConvNet(image_size)
                criterion = nn.CrossEntropyLoss()
